...
that listens on a Virtual IP + Protocol + Port + FQDN and somewhere (Internet, interface of a CE or a K8S namespace).
Once traffic is matched, Network and Security services are applied on the TCP session or the HTTP transaction.
Then traffic is Load Balanced to a pool of ORIGIN servers.
...
it's possible with a Platform as a Service.

Because ADN is one giant Kubernetes cluster, named a ``Virtual Kubernetes``,
F5 XC is designed to execute containers in all or in a group of Edges.
These containers can be :
    - part of an application (micro-service, front-end, middle-end, ...). For example `NGINX Unit <https://unit.nginx.org/>`_ running Flask restfull server
